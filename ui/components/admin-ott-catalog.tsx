...
export function AdminOttCatalog() {
  ...
  useEffect(() => {
    Promise.all([getMovies(0, 500), getAllSeries()])
      .then(([movies, series]) => {
        ...
        const catalog = [...toItems(movies.movies || [], "movie"), ...toItems(series, "series")]
        setItems(catalog)
        setSelected(catalog[0]?.provider || "")
      })
      .catch((error) => console.error("Failed to load OTT catalog", error))
