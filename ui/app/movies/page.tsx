...
export default function MoviesPage() {
  ...
  return <div className="min-h-screen bg-background"><Header /><main className="mx-auto max-w-7xl px-4 py-8">
    <div className="mb-6 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
      <div>
        <h1 className="flex items-center gap-2 text-3xl font-bold"><Film className="text-primary" /> Movies</h1>
        <p className="mt-2 text-muted-foreground">{total} movie{total === 1 ? "" : "s"}{watchAvailable ? " with watch-online links" : " in the catalog"}.</p>
      </div>
      <div>
        <RequestTitleDialog />
      </div>
