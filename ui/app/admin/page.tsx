...
export default function AdminPage() {
    ...
    useEffect(() => {
        ...
        async function loadCounts() {
            setLoading(true)
            try {
                const [moviesRes, theatersRes, usersRes, requestsRes] = await Promise.allSettled([
                    getMovies(0, 1),
                    getTheaters(),
                    getAllUsers(),
                    getAdminRequests(),
                ...
                setCounts({
                    movies: moviesRes.status === "fulfilled"
                        ? moviesRes.value?.total ?? moviesRes.value?.movies?.length ?? moviesRes.value?.length ?? "—"
                        : "—",
