...
export default function AdminTheaterPage() {
  ...
  return <div className="min-h-screen bg-background"><Header /><main className="mx-auto max-w-5xl space-y-6 px-4 py-8">
    ...
    <Card><CardHeader className="flex-row items-center justify-between"><CardTitle>Ticket platforms</CardTitle><Button variant="outline" size="sm" onClick={() => change("ticketPlatforms", [...(theater.ticketPlatforms || []), { name: "", url: "" }])}><Plus className="mr-1 h-4 w-4" />Add platform</Button></CardHeader><CardContent className="space-y-3">{(theater.ticketPlatforms || []).map((platform, index) => <div className="flex gap-2" key={index}><Input value={platform.name} placeholder="BookMyShow" onChange={(e) => updatePlatform(index, "name", e.target.value)} /><Input value={platform.url} placeholder="https://…" onChange={(e) => updatePlatform(index, "url", e.target.value)} /><Button variant="ghost" size="icon" onClick={() => change("ticketPlatforms", (theater.ticketPlatforms || []).filter((_, i) => i !== index))}><Trash2 className="h-4 w-4" /></Button></div>)}</CardContent></Card>
  </main></div>
}
